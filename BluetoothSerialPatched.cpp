...
namespace BluetoothSerialPatched_namespace {
    ...
    static bool btSetPin() {
        ...
        if (_isPinSet) {
            ...
            return (esp_bt_gap_set_pin(pin_type, _pin_code_len, _pin_code) == ESP_OK);        
        }
        return false;
    }
    ...
    static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
    {
        ...
        case ESP_SPP_OPEN_EVT:  // Enum 26 - When SPP Client connection open
            ...
            if (!_spp_client){
                ...
                esp_spp_disconnect(param->open.handle); // Used to be  _spp_client = param->srv_open.handle; Is this a bugfix?
            }
        ...
        default: log_i("ESP_SPP_* event #%d unhandled", event);
            break;
        }
        ...
        switch(event){
            ...
            case ESP_BT_GAP_PIN_REQ_EVT:  // Enum 5 - Legacy Pairing Pin code request
                ...
                if (param->pin_req.min_16_digit && _pin_code_len < 16) {
                    // Server requires a minimum of 16 digits, but we have fewer.
    ...
    static bool _init_bt(const char *deviceName) {
        ...
        // This code only works on IDF 3.3.2 or something.
        //
        ...
        // This code only works on IDF 3 or higher.
        //
...
using namespace BluetoothSerialPatched_namespace;
...
BluetoothSerialPatched::BluetoothSerialPatched(void) {
 ...
 */
bool BluetoothSerialPatched::begin(String localName, bool isMaster, bool disableBLE) {
    _isMaster = isMaster;
    if (localName.length()){
...
int BluetoothSerialPatched::available(void) {
    if (_spp_rx_queue == NULL){
        return 0;
    }
...
int BluetoothSerialPatched::peek(void) {
    uint8_t c;
...
bool BluetoothSerialPatched::hasClient(void) {
    return _spp_client > 0;
}

int BluetoothSerialPatched::read() {
 ...
 */
void BluetoothSerialPatched::setTimeout(int timeoutMS) {
    Stream::setTimeout(timeoutMS);
    this->timeoutTicks=timeoutMS / portTICK_PERIOD_MS;
}

size_t BluetoothSerialPatched::write(uint8_t c) {
    return write(&c, 1);
}

size_t BluetoothSerialPatched::write(const uint8_t *buffer, size_t size) {
    if (!_spp_client) {
        return 0;
    }
...
void BluetoothSerialPatched::flush() {
    if (_spp_tx_queue != NULL) { 
...
void BluetoothSerialPatched::end() {
    _stop_bt();
}
 ...
 */
void BluetoothSerialPatched::memrelease() {
    esp_bt_mem_release(ESP_BT_MODE_BTDM);
}

void BluetoothSerialPatched::onData(BluetoothSerialDataCb cb){
    custom_data_callback = cb;
}

void BluetoothSerialPatched::onConfirmRequest(ConfirmRequestCb cb) {
    confirm_request_callback = cb;
}

void BluetoothSerialPatched::onKeyRequest(KeyRequestCb cb) {
   key_request_callback = cb;
}

void BluetoothSerialPatched::respondPasskey(uint32_t passkey) {
    esp_bt_gap_ssp_passkey_reply(current_bd_addr, true, passkey);
}

void BluetoothSerialPatched::onAuthComplete(AuthCompleteCb cb) {
    auth_complete_callback = cb;
}

void BluetoothSerialPatched::confirmReply(boolean confirm) {
    esp_bt_gap_ssp_confirm_reply(current_bd_addr, confirm);  
}


esp_err_t BluetoothSerialPatched::register_callback(esp_spp_cb_t callback) {
...
void BluetoothSerialPatched::enableSSP() {
...
void BluetoothSerialPatched::enableSSP(bool inputCpability, bool outputCapability) {
...
// This must be called before calling begin, otherwise has no effect!
void BluetoothSerialPatched::disableSSP() {
    _enableSSP = false;
}

bool BluetoothSerialPatched::setPin(const char *pin, uint8_t pin_code_len) {
...
bool BluetoothSerialPatched::connect(String remoteName)
{
    bool retval = false;
 ...
 */
bool BluetoothSerialPatched::connect(uint8_t remoteAddress[], int channel, esp_spp_sec_t sec_mask, esp_spp_role_t role) {
    bool retval = false;
...
bool BluetoothSerialPatched::connect()
{
    if (!isReady(true, READY_TIMEOUT)) {
        return false;
    }
...
bool BluetoothSerialPatched::disconnect() {
    if (_spp_client) {
        flush();
        log_i("disconnecting");
...
bool BluetoothSerialPatched::unpairDevice(uint8_t remoteAddress[]) {
    if (isReady(false, READY_TIMEOUT)) {
...
bool BluetoothSerialPatched::connected(int timeout) {
 ...
 */
bool BluetoothSerialPatched::isClosed() {
    return xEventGroupGetBits(_spp_event_group) & SPP_CLOSED;
}

bool BluetoothSerialPatched::isReady(bool checkMaster, int timeout) {
 ...
 */
BTScanResults* BluetoothSerialPatched::discover(int timeoutMs) {
    scanResults.clear();
 ...
 */
bool BluetoothSerialPatched::discoverAsync(BTAdvertisedDeviceCb cb, int timeoutMs) {
    scanResults.clear();
...
void BluetoothSerialPatched::discoverAsyncStop() {
    esp_bt_gap_cancel_discovery();
    advertisedDeviceCb = nullptr;
}

/** @brief      Clears scanresult entries */
void BluetoothSerialPatched::discoverClear() {
    scanResults.clear();
}
 ...
 */
BTScanResults* BluetoothSerialPatched::getScanResults() {
    return &scanResults;
}

BluetoothSerialPatched::operator bool() const {
    return true;
}
 ...
 */
std::map<int, std::string> BluetoothSerialPatched::getChannels(const BTAddress &remoteAddress) {
 ...
 */
void BluetoothSerialPatched::getBtAddress(uint8_t *mac) {
 ...
 */
BTAddress BluetoothSerialPatched::getBtAddressObject() {
 ...
 */
String BluetoothSerialPatched::getBtAddressString() {
...
void BluetoothSerialPatched::requestRemoteName(uint8_t remoteAddress[]) {
    if (isReady(false, READY_TIMEOUT)) {
...
bool BluetoothSerialPatched::readRemoteName(char rmt_name[ESP_BT_GAP_MAX_BDNAME_LEN + 1]) {
    if (_rmt_name_valid) {
...
void BluetoothSerialPatched::invalidateRemoteName() {
    _rmt_name_valid = false;
}

int BluetoothSerialPatched::getNumberOfBondedDevices() {
...
int BluetoothSerialPatched::getBondedDevices(uint dev_num, esp_bd_addr_t *dev_list) {
...
bool BluetoothSerialPatched::deleteBondedDevice(uint8_t *remoteAddress) {
...
void BluetoothSerialPatched::deleteAllBondedDevices() {
    if (!isReady(false, READY_TIMEOUT)) {
